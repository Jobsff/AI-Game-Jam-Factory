...
## Files Allowed
- `08_Prefab_Library/system-prefabs/score/ScoreSystem.js`
- `tests/` 中仅与 `score` 直接相关的测试文件

## Public Interface
`new ScoreSystem(config)`，公开方法：add(amount), sub(amount), get(), enable(), disable(), reset(), destroy()。必须命名导出事件常量、主类并默认导出主类。
...
## Non-goals
不显示分数，不持久化最高分，不决定奖励规则。 禁止引入框架、构建链、全局可变状态或运行时 npm 依赖。
